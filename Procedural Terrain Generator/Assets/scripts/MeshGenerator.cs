...
public static class MeshGenerator 
{
    ...
    public static MeshData GenerateTerrainMesh(float[,] heightMap, MeshSettings meshSettings, int levelOfDetail)
    {
        ...
        int skipIncrement = (levelOfDetail == 0) ? 1 : levelOfDetail * 2; //so that the level of detail is always at least 1
        ...
        MeshData meshData = new MeshData(numVerticesPerLine, skipIncrement, meshSettings.lowPolyMode);
        ...
        for (int y = 0; y < numVerticesPerLine; y++)
        {
            for (int x = 0; x < numVerticesPerLine; x++)
            {
                ...
                if (!isSkippedVertex)
                {
                    ...
                    Vector2 percent = new Vector2 (x-1, y-1) / (numVerticesPerLine -3); //makes sure UVs are properly centered
                    ...
                    if (createTriangle)
                    {
                        ...
                        meshData.AddTriangle(a, d, c); //creates triangles out of 3 points 
...
public class MeshData
{
    ...
    public MeshData(int numVerticesPerLine, int skipIncrement, bool useFlatShading)
    {
        ...
        int numMeshEdgeVertices = (numVerticesPerLine - 2) * 4 - 4; //-4 for when it double counts the corners
        ...
        int numMainVertices = numMainVerticesPerLine * numVerticesPerLine;
        ...
        outOfMeshVertices = new Vector3[numVerticesPerLine * 4 - 4];// times 4 for all the vertices around the mesh and plus 4 for the corners
    ...
    {
        if (a < 0 || b < 0 || c < 0) // checking to see if the triangle is part of a border vertex
        {
